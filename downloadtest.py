...
"""
import glob, os, shutil, sys, tempfile, threading, time
from concurrent.futures import ThreadPoolExecutor, as_completed

import yt_dlp

from config import (BATCH_WORKERS, BATCH_FRAGMENT_THREADS,
                    YT_PLAYER_CLIENT, YT_PLAYER_CLIENT_FALLBACK)
from downloader import _DL_BASE, _client_args
...
def _fetch_entries(url: str) -> list | None:
    flat_opts = {
...
def _dl_one(entry: dict, tmp_dir: str) -> str | None:
    ...
    for clients in (YT_PLAYER_CLIENT, YT_PLAYER_CLIENT_FALLBACK):
        opts = {
            **_DL_BASE,
    ...
    try:
        ...
        if entries is None:
            ...
            with ThreadPoolExecutor(max_workers=BATCH_WORKERS) as executor:
                futs = {executor.submit(_worker, e, i + 1): i
